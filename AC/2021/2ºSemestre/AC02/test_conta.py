...
@pytest.mark.parametrize('valor,checa_extrato', [(-300, False), (0, False), (300, False), (500, False),
                                                 (-300, True), (0, True), (300, True), (400, False), (400, True),
                                                 (500, True)])
...
def test_conta_transferir(valor, checa_extrato, ativa_origem, ativa_destino):
    ...
    try:
        ...
        if not ativa_origem or not ativa_destino:
            ...
            assert c1.saldo == (saldo_inicial_1 - valor) and c2.saldo == (
                        saldo_inicial_2 + valor), 'A transferência deve descontar o valor do saldo da conta original e acrescentar o valor ao saldo da conta de destino'
            if checa_extrato:
...
def test_conta_tirar_extrato_sem_operacoes():
    try:
        ...
        assert lista[0] == ('saldo inicial',
                            saldo_inicial), 'O método tirar_extrato() retorna uma lista de operações com valores diferentes aos esperados'
...
def test_conta_tirar_extrato_apos_operacoes():
    ...
    try:
        ...
        if len(lista) == 3:
            ...
            'saque',
            100), 'O método tirar_extrato() retorna uma lista de operações com valores diferentes aos esperados'
        else:
